...
const ChatChatSection: React.FC<ChatChatSectionProps> = ({
  ...
  // Start session & show first AI message
  useEffect(() => {
    const startSession = async () => {
      try {
        ...
        if (!res.ok) throw new Error("خطا در ایجاد جلسه چت");

        const data = await res.json();
        // Save sessionId
        setSessionId(data.sessionId);
        // Show the initial AI message returned from server
        if (data.message) {
          setMessages([
            {
              sender: 'ai',
        ...
        setMessages([
          {
            sender: "ai",
            text: "خطا در شروع جلسه چت. لطفا دوباره تلاش کنید.",
            color: "red",
          },
        ]);
      }
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      try {
        const res = await fetch(`${BASE_URL}/api/v1/chat/messages?sessionId=${sessionId}`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        // Append history below the initial AI message (if any)
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessages(prev => [...prev, { sender: 'ai', text: errorMsg, color: 'red' }]);
        return;
      }
