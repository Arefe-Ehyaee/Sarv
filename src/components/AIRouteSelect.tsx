    ...
    <div className="flex flex-col justify-between font-myVazirMedium text-lg bg-primary-100">
      <ul className="space-y-[20px] text-primary-600">
        {[
          ...
          { to: "/searchEngine", icon: <Clipboard />, label: "تست روانشناسی" },
          { to: "/searchEngine", icon: <Calendar />, label: "جلسه با تراپیست" },
          { to: "/searchEngine", icon: <Music />, label: "موسیقی" },
          { to: "/searchEngine", icon: <Hexa />, label: "کارت انگیزه" }
        ].map((item, idx) => (
          <li key={idx}>
            <NavLink
              to={item.to}
