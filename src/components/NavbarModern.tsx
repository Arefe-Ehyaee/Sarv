  ...
  return (
    <nav className="fixed top-0 left-1/2 -translate-x-1/2 w-[60%] bg-[#eeeeee] z-50 rounded-full mt-2">
      <div className="mx-auto px-6 sm:px-8 lg:px-4 h-16 flex items-center justify-between max-w-6xl">
        ...
        {/* Center: nav links */}
        <ul className="hidden sm:flex items-center gap-8 text-sm desktop:text-base font-myYekanMedium text-gray-800">
        ...
        <div>
          {user ? (
            <div className="relative">
              ...
              {profileOpen && (
                <ul className="absolute left-0 mt-2 bg-white border border-primary-600 font-myYekanRegular rounded-lg shadow-md px-2 py-2 w-48 text-right">
                  <li className="px-4 py-2 hover:bg-gray-100 rounded-lg" onClick={() => { setProfileOpen(false); navigate('/dashboard'); }}>
                    پروفایل کاربری
                  </li>
                  {/* <li className="border-t border-gray-200 mt-1"></li> */}
                  <li className="px-4 py-2 text-error-500 hover:bg-gray-100  rounded-lg"
