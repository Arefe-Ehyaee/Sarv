...
export default function SideNavbar() {
  ...
  return (
    <div className={`fixed top-0 right-0 z-10 bg-primary-100 ${collapsed ? "w-[80px]" : "w-[340px]"} min-h-screen flex flex-col justify-between px-[20px] py-[20px] border-l border-primary-200 transition-all duration-300`}>
  ...
  <span className="flex flex-row items-center text-primary-600 font-myPeydaRegular text-[30px]">
    ...
    {!collapsed && <span className="ml-2">سرو بات</span>}
      ...
      <CustomButton
        text={collapsed ? "" : "ورود"}
        iconsrc={login}
        className={`bg-primary-400 h-[44px] ${collapsed ? "w-[44px]" : "w-[94px]"} text-background-BG text-base font-myVazirSemibold flex justify-center items-center`}
      />
    </div>
  );
}
